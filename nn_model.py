...
def main():
    ...
    if os.path.exists(model_file):
        ...
        model = Sequential() #<- an empty model we can insert things sequential into 
        model.add(Dense(units = 100, input_dim = 1000, activation = 'relu')) #layer of 100 nuerson (how?) 
        model.add(Dense(units = 50, activation = 'relu')) #layer of 100 nuerson (how?)
        model.add(Dense(units = 50, activation = 'relu'))
        model.add(Dense(units = 1, activation = 'sigmoid'))

        #we've told the model ok here are your layers! 
        # we still need to tell the model how to "optimize" #CS221 
